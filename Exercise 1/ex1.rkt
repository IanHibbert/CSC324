...
(define (search-table table item)
  (cond [(empty? table) '()]
        [(equal? (find (first table) item) #t)
         (cons (first table)
               (search-table (rest table) item))]
        [else (search-table (rest table) item)]
        )
  )

(define (higher-search-table table item)
  (filter (lambda (x)
            (find x item)) table)
  )

(define (find lst item)
  (if (empty? lst)
      #f
...
(define (search-table-2 table item [pos 0])
  (define lst (search-table table item))
  
  (filter (lambda (x) (equal? (list-ref x pos) item))
          (filter (lambda (x) (< pos (length x)))
                  lst)
          )
  )
